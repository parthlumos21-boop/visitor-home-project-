    ...
    <View className="flex-1 bg-gray-50">
      ...
      {isLoading ? (
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="mt-3 text-sm text-gray-500">Loading invitations</Text>
        </View>
      ) : (
        <ScrollView
          className="flex-1"
        ...
        >
          {error ? (
            ...
            <View className="mt-10 items-center">
              <Text className="text-base text-gray-500">No sent invitations found.</Text>
            </View>
          ) : (
            visits.map((visit) => (
        ...
        <Pressable className="flex-1 items-center justify-center bg-black/50 px-5" onPress={() => setDetailsVisit(null)}>
          <Pressable className="max-h-[86%] w-full rounded-lg bg-white" onPress={(event) => event.stopPropagation()}>
            <View className="flex-row items-center justify-between border-b border-gray-200 px-4 py-3">
              <Text className="text-lg font-bold text-gray-950">Invitation Details</Text>
